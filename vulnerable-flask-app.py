...
def main_page():
    ...
    entry_path = os.path.join(current_directory, 'entries')
    entry_paths_list = [os.path.join(entry_path, file) for file in os.listdir(entry_path)]  # list of all entry paths

    photo_path = os.path.join(current_directory, 'static')
    # photo_paths_list = [os.path.join(photo_path, file) for file in os.listdir(photo_path)]

    list_entries = sorted(entry_paths_list, key=os.path.getctime)
    top_stories = []
    for entry in list_entries[-1:-5:-1]:
        photo_name = os.path.join(photo_path, os.path.basename(entry))  # NOTE: this photo name has wrong extension .md
        # photo_name[0:-3] we delete the last 3 letters which are the extension .md
        if os.path.exists(''.join([photo_name[0:-3], '.jpg'])):
            photo_relpath = os.path.relpath(''.join([photo_name[0:-3], '.jpg']), current_directory)
            top_stories.append([photo_relpath, markdown2.markdown(get_entry(entry)[0:100])])

        elif os.path.exists(''.join([photo_name[0:-3], '.jpeg'])):
            photo_relpath = os.path.relpath(''.join([photo_name[0:-3], '.jpeg']), current_directory)
            top_stories.append([photo_relpath, markdown2.markdown(get_entry(entry)[0:100])])

    return render_template ("news_letter.html", top_stories=top_stories)
